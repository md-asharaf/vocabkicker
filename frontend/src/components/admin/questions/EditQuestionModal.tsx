...
export default function EditQuestionModal({ question, isPending, onSubmit, onCancel }: Props) {
  const [word, setWord] = useState(question.word);
  const [mnemonic, setMnemonic] = useState(question.mnemonic);
  ...
  return (
    <ModalShell>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-semibold text-white tracking-tight">Edit Question</h3>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div>
          ...
          <input className={inputCls} placeholder="e.g. Abundant" value={word} onChange={e => setWord(e.target.value)} required />
        </div>
        <div>
          <label className={labelCls}>Mnemonic</label>
          <input className={inputCls} placeholder="e.g. A bun dance" value={mnemonic} onChange={e => setMnemonic(e.target.value)} required />
        </div>
        <div>
          <label className={labelCls}>Definition</label>
          <textarea className={`${inputCls} min-h-[100px] resize-none`} placeholder="e.g. Existing or available in large quantities; plentiful." value={definition} onChange={e => setDefinition(e.target.value)} required />
        </div>
