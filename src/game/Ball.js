 ...
 * Ball.js
 *
 * 3D football: classic pentagon-patch texture, bezier arc flight,
 * and curve-shot physics (Magnus-effect approximation).
 *
 * Curve mechanic
 * ──────────────
 * curveFactor ∈ [-1, +1] shifts the bezier control point laterally:
 *   +1 → ball swings right  (outswinger for right-footer aiming right)
 *   -1 → ball swings left   (inswinger / banana)
 *   0  → straight shot
 *
 * The shift magnitude grows with the distance to the goal so long-range
 * shots curve more visibly than short ones.
 */
import * as THREE from 'three';
import { BALL_POS } from '../constants.js';
...
function makeBallTexture() {
  ...
  // Black pentagon patches (classic Telstar / Adidas layout)
...
export class Ball {
  ...
  reset() {
    ...
    this.shadow.material.opacity = 0.35;
    this._flying    = false;
    this._falling   = false;
    this._isCaught  = false;
    this._t         = 0;
  ...
  /**
   * Launch the ball toward `dst` with optional curve.
   * @param {THREE.Vector3} dst  target point in goal
   * @param {number} curveFactor -1..+1
   * @param {Function} onLand   called when ball reaches destination
   */
  kick(dst, curveFactor, onLand) {
  ...
  update(dt) {
    ...
    this.shadow.material.opacity = Math.max(0.05, 0.35 * groundScale);
