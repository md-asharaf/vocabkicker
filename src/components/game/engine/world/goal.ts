...
function netPanel(w: number, h: number, rx = 0, ry = 0, x = 0, y = 0, z = 0, segmentsX = 1, segmentsY = 1) {
  const S   = 256;
 ...
 * @returns {THREE.Group}
 */
export function createGoal(scene: THREE.Scene) {
  ...
  const sb = (x: number, y: number) => bar(GOAL_D, Math.PI / 2, 0, x, y, GOAL_Z - hd);
